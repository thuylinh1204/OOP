...
/**
 * An adjacency map structure for a directed graph. A double map structure is
 * used to represent the directed graph. The vertices are stored in the a map.
 * The outgoing and incoming edges are stored in two maps of the corresponding
 * vertex. The double map structure can be used since the vertices added are
 * unique i.e. with unique <code>String</code> labels. This structure provides
 * similar performance to an adjacency matrix where the
 * {@link #getEdge(Vertex u, Vertex v)} method can achieve O(1) by performing
 * lookup on the first and second map respectively.
 *
 * @param <V> Vertex type
 * @param <E> Edge type
 */
public class AdjacencyMapDigraph<V, E> implements Graph<V, E> {
    /**
     * Concrete implementation of {@link Vertex}. A {@link DVertex} object stores a
     * {@link V} element and its edges. Edges are implemented as
     * {@link LinkedHashMap} to provide O(1) lookup and also maintain the insertion
     * order.
     */
    ...
    /**
     * Concrete implementation of {@link Edge}. A {@link DEdge} object stores an
     * {@link E} element and its {@link V} end vertices.
     */
    ...
    @Override
    public synchronized Edge<E, V> insertEdge(Vertex<V> u, Vertex<V> v, E element)
            throws InvalidVertexException, InvalidEdgeException {
        if (getEdge(u, v) == null) {
            DVertex startVertex = validateVertex(u);
    ...
    @Override
    public synchronized Edge<E, V> insertEdge(V uElement, V vElement, E eElement)
            throws InvalidVertexException, InvalidEdgeException {
    ...
    public void inverseGraph() {
        ...
        for (Edge<E, V> e : edges) {
            Vertex<V>[] vertexes = e.vertices();
            this.insertEdge(vertexes[1], vertexes[0], e.element());
        }
